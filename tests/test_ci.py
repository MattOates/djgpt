    ...
    with mock.patch('openai.ChatCompletion.create') as mocked_create:
        ...
        mocked_create.return_value = {
            'choices': [
                {
                    'message': {
                        'content': '[{"artist": "Mock Artist", "trackname": "Mock Song"}]'
                    }
                }
            ]
        }
        ...
        response = openai.ChatCompletion.create(
            model="gpt-4",
            messages=[{"role": "user", "content": "Recommend music"}]
        )
        
        # Verify we get the expected response
        assert response['choices'][0]['message']['content'] == '[{"artist": "Mock Artist", "trackname": "Mock Song"}]'
